...
- class 는 문법으로 존재하며, 실무에서 직접 class를 만드는 경우는 극히 드물다.
- 백엔드를 js로 만들때는 이해 필수입니다.
...
- constructor 메서드 : 속성의 초기값을 셋팅해 줄 수 있다.
...
- 메서드는 객체의 기능을 말한다.(객체에 만든 함수를 메서드 라고 한다.)
...
class BankAccount {
  ...
  showBalance() {
    this.balance; // private: 내부에서는 활용가능
  }
}

const iu: BankAccount = new BankAccount("아이유", 55478);
iu.user;
iu.balance; // private 비공개 데이터 Error
```
...
class Cat extends Animal {
  ...
  show() {
    this.eye; // publid 이라서 마음대로 접근
    this.nose; // Error, private 이라서 아무도 접근 못함
    this.lips; // protected 상속받은 대상(자식)이라 사용가능.
  }
}
// 인스턴스 객체 생성
const cat = new Cat();
...
cat.lips; // Error :  protected 이라서 밖에서는 못 접근..?
```
