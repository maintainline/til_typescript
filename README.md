...
- 타입스크립트 파일은 js 가 아니라서 웹 브라우저 및 Node.js 에서 실행이 안된다
- 타입스크립트로 작성하면 자동으로 js 파일로 변역되어 진다.
- 그 결과로 만들어진 js 를 실행한다.

## 3. Typescript 특징

- 코딩 중에 오류를 발견하기 쉽다.(디버깅이 수월하다)
- js는 코딩중에 오류를 발견하기 어려움 (심각한 문제가 발생함)

## 4. Typescript 프로젝트 셋팅

- Node.js 를 설치하고 나면 자동으로 `npm`이 설치 됩니다.
...
- tsc 명령어 : TypeScript Complie
- `tsc 경로/파일명.ts` ===> `node 경로/파일명.js`
- 위의 두 과정을 하나로 실행 가능하도록 지원하는 npm
