...
## 4. 원본 배열이 변경되어 지는 이미 만들어진 기능 (메소드)
...
fruitsArr.splice(1, 2); // "사과" ex> 1번 인덱스로부터 2번째 까지 삭제//
// 원본 배열이 훼손된다.!주의사항!
```

- sort : 배열의 요소를 정렬해줌(가나다, abc, 123 순)
...
fruitsArr.sort(); // a, e, o, r,k
// 원본 배열이 훼손된다.!주의사항!

const numArr: number[] = [2, 3, 9, 7, 5];
fruitsArr.sort(); // 2,3,5,7,9
// 원본 배열이 훼손된다.!주의사항!

const numArr: number[] = [1, 12, 3, 19, 7, 5];
fruitsArr.sort(); // 1, 12, 19, 3, 5, 7
// 위의 결과는 일반적이지 않음  앞 글자를 따져서 1-12-19 순서로감
// 올림 차순으로 정렬하려면 추가 작업이 필요
const numArr: number[] = [1, 12, 3, 19, 7, 5];
numArr.sort((a, b) => a - b); // 1, 3, 5, 7, 12, 19

//내림차순 으로 정렬하려면 추가 작업이 필요
const numArr: number[] = [1, 12, 3, 19, 7, 5];
...
- 데이터 불변성(immutabillity) 을 유지하는 메소드 (`매우 중요~!`)
...
```js
const 원본배열 = [1, 2, 3, 4];
const 복사본 = 원본배열.map(function (요소, 인덱스, 원본배열) {
  return 요소;
}); // [1, 2, 3, 4]
...
const 학생목록배열 = 학생목록배열.filter(function (요소, 인덱스, 원본배열) {
  if (요소.study) {
    return 요소;
  }
});
...
/// [2, 4, 5, 8, 2, "a", "b", "c"] 배열 합치기
```

### 5.4. join : 배열을 하나의 문자열로 배열을 표현하기 (자주사용)
...
```js
const numArr = [1, 2, 3];
const result = numArr.inculdes(3); // true
```

```ts
const numArr: number[] = [1, 2, 3];
const result: boolean = numArr.inculdes(3); // true
```
