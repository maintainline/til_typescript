...
- 문제 상황 1 개선을 위해 enum 사용.

```ts
// 백엔드 데이터 호출 관련한 타입 정의
// 불편함 : 오타 발생 소지 개선
// 불편함 : 하나의 값을 변경하면 모두 찾아서 변경 개선
enum State {
  LOADING = "Loading",
  SUCCESS = "Success",
  ERROR = "Error",
...
- 문제 상황 2
  - `값은 코드 개선으로 enum 을 사용하여 효율성`을 올려줌.
  - 여전히 `키명은 오타`를 내거나,`키명 변경시` 적용 부분은 개선하지 못함.
...
// 자동으로 키명을 받아올 수 있다면? 오타 줄임, 코드개선 효율적
// 위의 코드와 같은 코드임.
type UserGetApiAuto = {
  // 맵핑을 사용하면 된다.
...
```ts
// 원하는 속성의 이름과 타입을 추출했는데 나는 옵셔널로 설정하고 싶을때?
/**
 * type UserGetApiAll4 = {
      getUser?: State;
      getPageNum?: State;
